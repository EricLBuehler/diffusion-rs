    ...
    match (&*src.storage_and_layout().0, &*dst.storage_and_layout().0) {
        (Storage::Cuda(src), Storage::Cuda(tgt)) => {
            ...
            if src.dtype() != dst.dtype() {
                diffuse_rs_common::bail!("DTypes must match!");
            }

            match src.dtype() {
                DType::F8E4M3 => {
                    let tgt = tgt.as_cuda_slice::<F8E4M3>()?;
                    let src = src.as_cuda_slice::<F8E4M3>()?;
                    driver::result::memcpy_dtod_sync(
                        *tgt.device_ptr(),
                        *src.device_ptr(),
                        src.len() * std::mem::size_of::<F8E4M3>(),
                    )
                    .w()?;
                }
                DType::BF16 => {
                ...
                DType::I16 => {
                    let tgt = tgt.as_cuda_slice::<i16>()?;
                    let src = src.as_cuda_slice::<i16>()?;
                    driver::result::memcpy_dtod_sync(
                        *tgt.device_ptr(),
                        *src.device_ptr(),
                        src.len() * std::mem::size_of::<i16>(),
                    )
                    .w()?;
                }
                DType::I32 => {
                    let tgt = tgt.as_cuda_slice::<i32>()?;
                    let src = src.as_cuda_slice::<i32>()?;
                    driver::result::memcpy_dtod_sync(
                        *tgt.device_ptr(),
                        *src.device_ptr(),
...
impl<T: GraphInput> Graph<T> {
    ...
    pub fn new(
        from_code: impl FnOnce(&T) -> diffuse_rs_common::core::Result<()>,
        device: &Device,
        input: T,
    ) -> diffuse_rs_common::core::Result<Self> {
        let cu_device = match &device {
            Device::Cuda(dev) => dev,
            _ => diffuse_rs_common::bail!("Must have CUDA device."),
        };
        ...
        // beforehand.
        {
            ...
            // Reduce
            let _ = diffuse_rs_common::nn::ops::softmax_last_dim(&x)?;
    ...
    pub fn replay(&self, input: T) -> diffuse_rs_common::core::Result<()> {
    ...
    pub fn output_dot<P: AsRef<Path>>(
        ...
        let command = Command::new("dot")
            ...
            .map_err(diffuse_rs_common::core::Error::debug)?
            .stdout;
    ...
    pub fn nodes(&self) -> diffuse_rs_common::core::Result<Vec<Node<'_>>> {
