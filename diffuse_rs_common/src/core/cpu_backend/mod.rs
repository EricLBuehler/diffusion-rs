...
use crate::core::{DType, Error, IntDType, Layout, Result, Shape, WithDType};
use float8::F8E4M3;
use half::{bf16, f16};
use rayon::prelude::*;
...
fn copy_strided_src_<T: Copy>(src: &[T], dst: &mut [T], dst_offset: usize, src_l: &Layout) {
    match src_l.strided_blocks() {
        crate::core::StridedBlocks::SingleBlock { start_offset, len } => {
        ...
        crate::core::StridedBlocks::MultipleBlocks {
            block_start_index,
            block_len: 1,
        } => {
...
impl Map2 for Conv1D<'_> {
    ...
    fn f<T: WithDType>(&self, inp: &[T], inp_l: &Layout, k: &[T], k_l: &Layout) -> Result<Vec<T>> {
        ...
        let (inp_s0, inp_s1, inp_s2) = crate::core::shape::dims3(inp_l.stride())?;
        let (k_s0, k_s1, k_s2) = crate::core::shape::dims3(k_l.stride())?;
        let l_out = p.l_out();
...
impl Map2 for Conv2D<'_> {
    ...
    fn f<T: WithDType>(&self, inp: &[T], inp_l: &Layout, k: &[T], k_l: &Layout) -> Result<Vec<T>> {
        let p = self.0;
        let inp = &inp[inp_l.start_offset()..];
        let (inp_s0, inp_s1, inp_s2, inp_s3) = crate::core::shape::dims4(inp_l.stride())?;
        let k = &k[k_l.start_offset()..];
        let (k_s0, k_s1, k_s2, k_s3) = crate::core::shape::dims4(k_l.stride())?;
...
impl MatMul {
    fn striding_error(&self, lhs_l: &Layout, rhs_l: &Layout, msg: &'static str) -> Error {
        Error::MatMulUnexpectedStriding(Box::new(crate::core::error::MatMulUnexpectedStriding {
            lhs_l: lhs_l.clone(),
            rhs_l: rhs_l.clone(),
            bmnk: self.0,
            msg,
...
impl Map2 for MatMul {
    ...
    fn f<T: 'static + WithDType + num_traits::Num + Copy>(
        ...
        let num_threads = crate::core::utils::get_num_threads();
...
impl BackendStorage for CpuStorage {
    ...
    fn to_dtype(&self, layout: &Layout, dtype: DType) -> Result<Self> {
        ...
        match (self, dtype) {
            ...
            (Self::U8(storage), DType::I8) => {
                let data = unary_map(storage, layout, |v| v as i8);
                Ok(Self::I8(data))
            }
            (Self::BF16(storage), DType::I8) => {
                let data = unary_map(storage, layout, |v| v.to_f32() as i8);
                Ok(Self::I8(data))
            }
            (Self::F16(storage), DType::I8) => {
                let data = unary_map(storage, layout, |v| v.to_f32() as i8);
                Ok(Self::I8(data))
            }
            (Self::F32(storage), DType::I8) => {
                let data = unary_map(storage, layout, |v| v as i8);
                Ok(Self::I8(data))
            }
            (Self::F64(storage), DType::I8) => {
                let data = unary_map(storage, layout, |v| v as i8);
                Ok(Self::I8(data))
            }
            (Self::U32(storage), DType::I8) => {
                let data = unary_map(storage, layout, |v| v as i8);
                Ok(Self::I8(data))
            }
            (Self::I16(storage), DType::I8) => {
                let data = unary_map(storage, layout, |v| v as i8);
                Ok(Self::I8(data))
            }
            (Self::I32(storage), DType::I8) => {
                let data = unary_map(storage, layout, |v| v as i8);
                Ok(Self::I8(data))
            }
            (Self::I64(storage), DType::I8) => {
                let data = unary_map(storage, layout, |v| v as i8);
                Ok(Self::I8(data))
            }
            (Self::F8E4M3(storage), DType::I8) => {
                let data = unary_map(storage, layout, |v| v.to_f32() as i8);
                Ok(Self::I8(data))
            }
        }
    ...
    fn powf(&self, layout: &Layout, e: f64) -> Result<Self> {
        ...
        match self {
            ...
            Self::I8(_) => Err(Error::UnsupportedDTypeForOp(DType::U8, "elu").bt()),
    ...
    fn binary_impl<B: BinaryOpT>(
        ...
        match (self, rhs) {
            ...
            (Self::I8(lhs), Self::I8(rhs)) => {
                let data = if B::I8_VEC {
                    binary_map_vec(lhs_l, rhs_l, lhs, rhs, B::i8, B::i8_vec)
                } else {
                    binary_map(lhs_l, rhs_l, lhs, rhs, B::i8)
                };
                Ok(Self::I8(data))
            }
            _ => {
