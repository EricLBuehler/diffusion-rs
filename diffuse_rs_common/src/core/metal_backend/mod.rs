...
use crate::core::conv::{ParamsConv1D, ParamsConv2D, ParamsConvTranspose1D, ParamsConvTranspose2D};
use crate::core::op::{BinaryOpT, CmpOp, ReduceOp, UnaryOpT};
use crate::core::{CpuStorage, CpuStorageRef, DType, Layout, Result, Shape};
use crate::metal_kernels::{BufferOffset, CallConvTranspose2dCfg, Kernels};
...
pub enum MetalError {
    ...
    Message(String),
    #[error(transparent)]
    KernelError(#[from] crate::metal_kernels::MetalKernelError),
    #[error("{0:?}")]
    LockError(LockError),
...
impl BackendStorage for MetalStorage {
    ...
    fn affine(&self, layout: &Layout, mul: f64, add: f64) -> Result<Self> {
        ...
        if layout.is_contiguous() {
            ...
            crate::metal_kernels::call_affine(
                &device.device,
                &command_buffer,
                &device.kernels,
                name,
            ...
            crate::metal_kernels::call_affine_strided(
                &device.device,
                &command_buffer,
                &device.kernels,
                name,
    ...
    fn powf(&self, layout: &Layout, pow: f64) -> Result<Self> {
        ...
        if layout.is_contiguous() {
            ...
            crate::metal_kernels::call_powf_strided(
                &device.device,
                &command_buffer,
                &device.kernels,
                name,
    ...
    fn elu(&self, layout: &Layout, alpha: f64) -> Result<Self> {
        ...
        if layout.is_contiguous() {
            ...
            crate::metal_kernels::call_elu_strided(
                &device.device,
                &command_buffer,
                &device.kernels,
                name,
    ...
    fn reduce_op(&self, op: ReduceOp, layout: &Layout, sum_dims: &[usize]) -> Result<Self> {
        ...
        let command_buffer = self.device.command_buffer()?;
        let src = buffer_o(&self.buffer, layout, self.dtype);
        crate::metal_kernels::call_reduce_strided(
            &device.device,
            &command_buffer,
            &device.kernels,
            name,
    ...
    fn to_dtype(&self, layout: &Layout, dtype: DType) -> Result<Self> {
        ...
        if layout.is_contiguous() {
            ...
            crate::metal_kernels::call_cast_contiguous(
                &device.device,
                &command_buffer,
                &device.kernels,
                kernel_name,
            ...
            crate::metal_kernels::call_cast_strided(
                &device.device,
                &command_buffer,
                &device.kernels,
                kernel_name,
    ...
    fn where_cond(
        ...
        crate::metal_kernels::call_where_cond_strided(
            &device.device,
            &command_buffer,
            &device.kernels,
            name,
    ...
    fn conv1d(
        ...
        let src = buffer_o(&self.buffer, layout, self.dtype);
        crate::metal_kernels::call_im2col1d_strided(
            &self.device.device,
            &command_buffer,
            &self.device.kernels,
            name,
    ...
    fn conv_transpose1d(
        ...
        let buffer = if USE_COL2IM_CONV1D_TR && can_use_col2im {
            ...
            let command_buffer = self.device.command_buffer()?;
            crate::metal_kernels::call_col2im1d(
                &self.device.device,
                &command_buffer,
                &self.device.kernels,
                name,
            ...
            crate::metal_kernels::call_conv_transpose1d(
                &self.device.device,
                &command_buffer,
                &self.device.kernels,
                name,
    ...
    fn conv2d(
        ...
        let src = buffer_o(&self.buffer, layout, self.dtype);
        crate::metal_kernels::call_im2col_strided(
            &self.device.device,
            &command_buffer,
            &self.device.kernels,
            name,
    ...
    fn conv_transpose2d(
        ...
        crate::metal_kernels::call_conv_transpose2d(
            &self.device.device,
            &command_buffer,
            &self.device.kernels,
            name,
    ...
    fn avg_pool2d(
        ...
        crate::metal_kernels::call_pool2d(
            &self.device.device,
            &command_buffers,
            &self.device.kernels,
            name,
    ...
    fn upsample_nearest2d(&self, inp_l: &Layout, out_w: usize, out_h: usize) -> Result<Self> {
        ...
        crate::metal_kernels::call_upsample_nearest_2d(
            &self.device.device,
            &command_buffer,
            &self.device.kernels,
            name,
    ...
    fn gather(&self, src_l: &Layout, ids: &Self, ids_l: &Layout, dim: usize) -> Result<Self> {
        if !ids_l.is_contiguous() {
            return Err(crate::core::Error::RequiresContiguous { op: "gather" }.bt());
        };
        ...
        crate::metal_kernels::call_gather(
            &device.device,
            &command_buffer,
            &self.device.kernels,
            name,
    ...
    fn scatter_add(
        ...
        if !ids_l.is_contiguous() || !src_l.is_contiguous() {
            return Err(crate::core::Error::RequiresContiguous { op: "scatter-add" }.bt());
        };
        let name = match (ids.dtype, self.dtype) {
        ...
        let ids = buffer_o(&ids.buffer, ids_l, ids.dtype);
        crate::metal_kernels::call_scatter_add(
            &self.device.device,
            &command_buffer,
            &self.device.kernels,
            name,
    ...
    fn index_select(&self, ids: &Self, src_l: &Layout, ids_l: &Layout, dim: usize) -> Result<Self> {
        ...
        let src = buffer_o(&self.buffer, src_l, dtype);
        let ids = buffer_o(&ids.buffer, ids_l, ids.dtype);
        crate::metal_kernels::call_index_select(
            &device.device,
            &command_buffer,
            &self.device.kernels,
            name,
    ...
    fn index_add(
        ...
        self.copy_strided_src(&mut acc, 0, l)?;
        if !ids_l.is_contiguous() || !src_l.is_contiguous() {
            return Err(crate::core::Error::RequiresContiguous { op: "index-add" }.bt());
        };
        let name = match (ids.dtype, self.dtype) {
        ...
        let src = buffer_o(&src.buffer, src_l, src.dtype);
        let ids = buffer_o(&ids.buffer, ids_l, ids.dtype);
        crate::metal_kernels::call_index_add(
            &self.device.device,
            &command_buffer,
            &self.device.kernels,
            name,
    ...
    fn matmul_with_alpha_beta(
        ...
        let dtype = match self.dtype {
            DType::F32 => crate::metal_kernels::GemmDType::F32,
            DType::F16 => crate::metal_kernels::GemmDType::F16,
            DType::BF16 => crate::metal_kernels::GemmDType::BF16,
            dtype => {
        ...
        crate::metal_kernels::call_mlx_addmm(
            &self.device.device,
            &command_buffer,
            &self.device.kernels,
            dtype,
    ...
    fn matmul_with_alpha(
        ...
        if self.dtype == DType::BF16 {
            ...
            crate::metal_kernels::call_mlx_gemm(
                &self.device.device,
                &command_buffer,
                &self.device.kernels,
                crate::metal_kernels::GemmDType::BF16,
                (b, m, n, k),
                lhs_l.stride(),
                lhs_l.start_offset() * self.dtype.size_in_bytes(),
                &self.buffer,
    ...
    fn copy2d(
        ...
        if src_s == d2 && dst_s == d2 {
            ...
            let kernel_name = match self.dtype {
                DType::F32 => crate::metal_kernels::copy2d::FLOAT,
                DType::F16 => crate::metal_kernels::copy2d::HALF,
                DType::BF16 => crate::metal_kernels::copy2d::BFLOAT,
                DType::I16 => crate::metal_kernels::copy2d::I16,
                DType::I32 => crate::metal_kernels::copy2d::I32,
                DType::I64 => crate::metal_kernels::copy2d::I64,
                DType::U32 => crate::metal_kernels::copy2d::U32,
                DType::U8 => crate::metal_kernels::copy2d::U8,
    ...
    fn copy_strided_src(&self, dst: &mut Self, dst_offset: usize, src_l: &Layout) -> Result<()> {
        ...
        if src_l.is_contiguous() && self.dtype == dst.dtype() {
            ...
            let kernel_name = match self.dtype {
                DType::F32 => crate::metal_kernels::unary::strided::copy::FLOAT,
                DType::F16 => crate::metal_kernels::unary::strided::copy::HALF,
                DType::BF16 => crate::metal_kernels::unary::strided::copy::BFLOAT,
                DType::I16 => crate::metal_kernels::unary::strided::copy::I16,
                DType::I32 => crate::metal_kernels::unary::strided::copy::I32,
                DType::I64 => crate::metal_kernels::unary::strided::copy::I64,
                DType::U32 => crate::metal_kernels::unary::strided::copy::U32,
                DType::U8 => crate::metal_kernels::unary::strided::copy::U8,
...
impl MetalStorage {
    ...
    pub fn binary(
        ...
        let (buffer, dtype) = if lhs_l.is_contiguous() && rhs_l.is_contiguous() && &op[..1] != "b" {
            use crate::metal_kernels::binary::contiguous;
            ...
            crate::metal_kernels::call_binary_contiguous(
                &device.device,
                &command_buffer,
                &device.kernels,
                kernel_name,
            ...
            use crate::metal_kernels::binary::strided;
            ...
            let buffer = device.new_buffer(el_count, dtype, op)?;
            crate::metal_kernels::call_binary_strided(
                &device.device,
                &command_buffer,
                &device.kernels,
                kernel_name,
...
impl BackendDevice for MetalDevice {
    ...
    fn location(&self) -> crate::core::DeviceLocation {
        crate::core::DeviceLocation::Metal {
    ...
    fn rand_uniform(
        ...
        let command_buffer = self.command_buffer()?;
        crate::metal_kernels::call_random_uniform(
            &self.device,
            &command_buffer,
            &self.kernels,
            name,
    ...
    fn rand_normal(
        ...
        let command_buffer = self.command_buffer()?;
        crate::metal_kernels::call_random_normal(
            &self.device,
            &command_buffer,
            &self.kernels,
            name,
