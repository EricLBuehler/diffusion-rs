...
impl crate::core::Module for PReLU {
    fn forward(&self, xs: &Tensor) -> Result<Tensor> {
        let weight = if self.is_scalar {
            ...
            if num_weights != num_channels {
                crate::bail!("error in prelu: unexpected number of channels for the input, got {num_channels}, weight dim is {num_weights}")
            }
...
pub fn prelu(num_channels: Option<usize>, vs: crate::nn::VarBuilder) -> Result<PReLU> {
    let init_ws = crate::nn::init::Init::Const(0.25);
