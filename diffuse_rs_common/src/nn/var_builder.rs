...
impl SimpleBackend for crate::core::npy::NpzTensors {
    fn get(
        &self,
        s: Shape,
        name: &str,
...
impl SimpleBackend for crate::core::pickle::PthTensors {
    fn get(
        &self,
        s: Shape,
        name: &str,
...
impl<'a> VarBuilder<'a> {
    ...
    pub fn from_npz<P: AsRef<std::path::Path>>(p: P, dtype: DType, dev: &Device) -> Result<Self> {
        let npz = crate::core::npy::NpzTensors::new(p)?;
    ...
    pub fn from_pth<P: AsRef<std::path::Path>>(p: P, dtype: DType, dev: &Device) -> Result<Self> {
        let pth = crate::core::pickle::PthTensors::new(p, None)?;
...
pub struct ShardedSafeTensors(crate::core::safetensors::MmapedSafetensors);
...
impl Backend for ShardedSafeTensors {
    ...
    fn get(
        ...
        let iterator = if dim == 0 {
            ...
            crate::bail!("Get sharded on dimensions != 0 or 1")
        };
    ...
    fn get_unchecked(&self, _name: &str, _dtype: DType, _dev: &Device) -> Result<Tensor> {
        crate::bail!("`get_unchecked` does not make sense for `ShardedSafeTensors`, use `get`.");
    }
