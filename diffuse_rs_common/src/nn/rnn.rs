...
use crate::core::{DType, Device, IndexOp, Result, Tensor};
...
impl RNN for LSTM {
    ...
    fn step(&self, input: &Tensor, in_state: &Self::State) -> Result<Self::State> {
        ...
        let in_gate = crate::nn::ops::sigmoid(&chunks[0])?;
        let forget_gate = crate::nn::ops::sigmoid(&chunks[1])?;
        let cell_gate = chunks[2].tanh()?;
        let out_gate = crate::nn::ops::sigmoid(&chunks[3])?;
...
impl RNN for GRU {
    ...
    fn step(&self, input: &Tensor, in_state: &Self::State) -> Result<Self::State> {
        ...
        let r_gate = crate::nn::ops::sigmoid(&(&chunks_ih[0] + &chunks_hh[0])?)?;
        let z_gate = crate::nn::ops::sigmoid(&(&chunks_ih[1] + &chunks_hh[1])?)?;
