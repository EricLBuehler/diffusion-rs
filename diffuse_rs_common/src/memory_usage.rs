...
fn get_available_memory_vm_stat() -> Result<usize> {
    ...
    let output = Command::new("vm_stat")
        .output()
        .map_err(|e| crate::core::Error::msg(format!("Failed to execute vm_stat: {}", e)))?;

    // Convert output to a string
    let output_str = String::from_utf8(output.stdout)
        .map_err(|e| crate::core::Error::msg(format!("Failed to parse output: {}", e)))?;
...
fn get_total_memory_vm_stat() -> Result<usize> {
    ...
    let output = Command::new("sysctl")
        .arg("hw.memsize")
        .output()
        .map_err(|e| crate::core::Error::msg(format!("Failed to execute sysctl: {}", e)))?;
    ...
    pub fn get_memory_available(&self, device: &Device) -> Result<usize> {
        match device {
            ...
            Device::Cuda(_) => {
                ...
                Ok(crate::core::cuda::cudarc::driver::result::mem_get_info()
                    .w()?
                    .0)
            }
