...
use crate::core::{DType, DeviceLocation, Layout, MetalError, Shape};
...
macro_rules! bail {
    ($msg:literal $(,)?) => {
        return Err($crate::core::Error::Msg(format!($msg).into()).bt())
    };
    ($err:expr $(,)?) => {
        return Err($crate::core::Error::Msg(format!($err).into()).bt())
    };
    ($fmt:expr, $($arg:tt)*) => {
        return Err($crate::core::Error::Msg(format!($fmt, $($arg)*).into()).bt())
    };
}
