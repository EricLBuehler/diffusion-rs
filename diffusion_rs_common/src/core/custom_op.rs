use crate::core::op::{BackpropOp, Op};
use crate::core::tensor::from_storage;
use crate::core::{CpuStorage, CudaStorage, Layout, MetalStorage, Result, Shape, Tensor};
use std::sync::Arc;
...
pub trait CustomOp1 {
    ...
    fn bwd(&self, _arg: &Tensor, _res: &Tensor, _grad_res: &Tensor) -> Result<Option<Tensor>> {
        Err(crate::core::Error::BackwardNotSupported { op: self.name() })
    }
}
...
impl InplaceOp1 for UgIOp1 {
    ...
    fn metal_fwd(&self, sto: &mut MetalStorage, layout: &Layout) -> Result<()> {
        ...
        use crate::metal_kernels::utils::EncoderProvider;

        let elem_count = layout.shape().elem_count();
        if sto.dtype() != crate::core::DType::F32 {
        ...
        let group_dims = crate::metal_kernels::utils::get_block_dims(b as u64, 1, 1);
        crate::metal_kernels::utils::set_param(encoder, 0, (sto.buffer(), 0usize));
